...
def rgb2Lin(cchannel):
	for i in range(np.shape(cchannel)[0]):
		for j in range(np.shape(cchannel)[1]):
			value = cchannel[i,j]
			if (value <= 0.04045):
				value = value / 12.92
			else:
	...
	return cchannel
	# if (cchannel <= 0.04045):
	# 	return cchannel / 12.92
	# else:
	# 	return(np.power((cchannel+0.055)/1.055,2.4))
	
def rgb2Lumi(rgb_img):
