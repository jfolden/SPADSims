...
parser.add_argument('--num_quant', default=100, type=int, help='for spatial fovea stuff')
parser.add_argument('--num_rand', default=50, type=int, help='how many pixels to pick per bin')
parser.add_argument('--check', default=False, action='store_true', help='check if the incoming bin is greater than the previous')
parser.add_argument('--smrt_check', default=False, action='store_true', help='set to use sorted coords instead of random')
...
fnames = ["C:\\Users\\Justin\\tensorflow_datasets\\downloads\\extracted\\NYUv2\\nyudepthv2\\val\\official\\01155.h5",
          "C:\\Users\\Justin\\tensorflow_datasets\\downloads\\extracted\\NYUv2\\nyudepthv2\\val\\official\\01171.h5",
          "C:\\Users\\Justin\\tensorflow_datasets\\downloads\\extracted\\NYUv2\\nyudepthv2\\val\\official\\00520.h5",
          "C:\\Users\\Justin\\tensorflow_datasets\\downloads\\extracted\\NYUv2\\nyudepthv2\\val\\official\\00539.h5",
          "C:\\Users\\Justin\\tensorflow_datasets\\downloads\\extracted\\NYUv2\\nyudepthv2\\val\\official\\00390.h5"]
...
for P in tqdm(range(num_files_to_sim),desc="Total Sim"):
    ...
    out_data_base_dirpath = 'D:/spatial_temporal/{}/{checks}'.format(daymonthyear,checks="win{}_quant{}_check{}{}/".format(args.win_size,args.num_quant,args.check,args.smrt_check) 
                                                                                                            if (args.mem_fovea and args.num_quant) 
    ...
    if args.local_scale:
        curve = helpers.local_scale(depth_gt,predDepth_gt_raw,n_pts=25)
    ...
    data_gt.update(mono_raw = predDepth_gt_raw, mono_color = predDepth_gt_color, scaling = curve)
    ...
    if args.mem_fovea:
        [quant_bins,quant_depths,bins] = foveation.quantize(predDepth_gt_raw,num_bins=args.num_quant)
        spatial_coords = foveation.rand_pixel_per_bin(bins,quant_bins,num_pix_per_bin=args.num_rand,smartChoice=args.smrt_check)
        ...
        fovea_full = foveation.spatial_windows(fovea_data,win_t_start,spatial_coords,args.n_tbins,quant_bins)
        ...
        filled_depth = foveation.spatial_average(decoded_depths_windows,spatial_coords,quant_bins,checkmin=args.check)
        ...
        data_mem.update(c_vals = fovea_full, decoded_depths = filled_depth, sparse_depths=decoded_depths_windows, abs_depth_errors=abs_errors_win, error_metrics=win_errors, window_size = args.win_size, spatial_coords = spatial_coords,quant_bins = quant_bins,quant_mono=quant_depths)
    ...
    if args.depth_fovea:
        ...
        if args.mem_fovea:
            n_winbin = int((10*args.ds_bins)/(args.win_size*data_gt['tbin_depth_res']))
